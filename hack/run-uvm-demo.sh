...
# One-shot bring-up of the counter-uvm demo. GKE/dev-env by default; for a
# local kind cluster use hack/run-uvm-demo-kind.sh (which sets the kind env
...
#
# The committed .ko.yaml base for cmd/ateom-uvm is debian:stable-slim, which
...
# hack/run-uvm-demo-kind.sh sets NO_DEV_ENV to skip this and use kind defaults.
...
if [[ -z "${KO_DOCKER_REPO}" ]]; then
  ...
  echo "       or use hack/run-uvm-demo-kind.sh for a local kind cluster)." >&2
  exit 1
fi
...
COLOR_RESET='\033[0m'
log() {
  echo -e "${COLOR_CYAN}[run-uvm-demo]: $*${COLOR_RESET}"
}
...
# --- 3. throwaway ko config overriding ONLY the ateom-uvm base -----------
...
OVERRIDE_KEY="github.com/agent-substrate/substrate/cmd/ateom-uvm"
if ! grep -q "^  ${OVERRIDE_KEY}:" "${KO_CONFIG_TMP}"; then
  echo "Error: could not find the cmd/ateom-uvm baseImageOverride line in .ko.yaml" >&2
  exit 1
fi
...
log "Using throwaway KO_CONFIG_PATH=${KO_CONFIG_PATH} (ateom-uvm base -> ${ATEOM_BASE_IMAGE})"
...
log "Applying the counter-uvm demo manifest..."
sed "s|\${BUCKET_NAME}|${BUCKET_NAME}|g" demos/counter/counter-uvm.yaml.tmpl \
...
cat <<EOF
  ...
  1. Wait for the ActorTemplate golden snapshot to be Ready:
       kubectl${KCTX_FLAG} wait --for=condition=Ready \\
         actortemplate/counter-uvm -n ate-demo-counter-uvm --timeout=600s
  ...
  2. Create + resume an actor (kubectl-ate; install with: go install ./cmd/kubectl-ate):
       kubectl ate${KCTX_FLAG} create actor my-counter-1 \\
         --template ate-demo-counter-uvm/counter-uvm
