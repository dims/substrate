...
# Bring up the counter-uvm demo on a local kind cluster. Wraps
# hack/run-uvm-demo.sh with the same kind-specific environment that
# hack/install-ate-kind.sh uses, so the image repo and snapshot bucket match what
# gets installed in the cluster. All arguments are forwarded to run-uvm-demo.sh.
...
exec "${ROOT}/hack/run-uvm-demo.sh" "$@"
